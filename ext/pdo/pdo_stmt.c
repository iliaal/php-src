...
static inline bool fetch_value(pdo_stmt_t *stmt, zval *dest, int colno, enum pdo_param_type *type_override)
{
	if (colno < 0) {
...
PHP_METHOD(PDOStatement, fetchObject)
{
	...
	if (!do_fetch(stmt, return_value, PDO_FETCH_CLASS, PDO_FETCH_ORI_NEXT, 0, NULL, NULL)) {
		PDO_HANDLE_STMT_ERR();
		RETVAL_FALSE;
	}
...
PHP_METHOD(PDOStatement, fetchColumn)
{
	...
	if (!fetch_value(stmt, return_value, col_n, NULL)) {
		PDO_HANDLE_STMT_ERR();
		RETURN_FALSE;
	}
}
...
PHP_METHOD(PDOStatement, fetchAll)
{
	...
	if (!do_fetch(stmt, &data, how | flags, PDO_FETCH_ORI_NEXT, 0, return_all, &fetch_failed)) {
		error = true;
	}
	...
	if (!error) {
		if ((how & PDO_FETCH_GROUP) || how == PDO_FETCH_KEY_PAIR ||
			...
			while (do_fetch(stmt, &data, how | flags, PDO_FETCH_ORI_NEXT, 0, return_all, &fetch_failed))
				;
		} else {
			array_init(return_value);
			do {
				zend_hash_next_index_insert_new(Z_ARRVAL_P(return_value), &data);
			} while (do_fetch(stmt, &data, how | flags, PDO_FETCH_ORI_NEXT, 0, NULL, &fetch_failed));
		}
		if (fetch_failed) {
			error = true;
		}
...
static void pdo_stmt_iter_move_forwards(zend_object_iterator *iter)
{
	...
	if (!do_fetch(stmt, &I->fetch_ahead, PDO_FETCH_USE_DEFAULT,
			PDO_FETCH_ORI_NEXT, 0, NULL, NULL)) {
