...
static bool pdo_stmt_verify_mode(pdo_stmt_t *stmt, zend_long mode, uint32_t mode_arg_num, bool fetch_all, zend_long default_fetch_type)
{
	...
	if (mode == PDO_FETCH_USE_DEFAULT) {
		flags = default_fetch_type & PDO_FETCH_FLAGS;
		mode = default_fetch_type & ~PDO_FETCH_FLAGS;
	}
...
PHP_METHOD(PDOStatement, fetch)
{
	...
	if (!pdo_stmt_verify_mode(stmt, how, 1, false, stmt->default_fetch_type)) {
		RETURN_THROWS();
	}
...
PHP_METHOD(PDOStatement, fetchAll)
{
	...
	PHP_STMT_GET_OBJ;
	if (!pdo_stmt_verify_mode(stmt, how, 1, true, stmt->default_fetch_type)) {
		RETURN_THROWS();
	}
	...
	zval *args, uint32_t variadic_num_args)
{
	...
	if (!pdo_stmt_verify_mode(stmt, mode, mode_arg_num, false, stmt->dbh->default_fetch_type)) {
		return false;
	}

	bool use_default = fetch_type == PDO_FETCH_USE_DEFAULT;
